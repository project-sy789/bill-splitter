      ...
      <div className="space-y-3 bg-white/40 px-5 py-4">
                    ...
                    {(['equally', 'percentage', 'exact'] as const).map((mode, idx) => {
                      ...
                      return (
                        <button
                          ...
                          className={`rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-[0.12em] transition-all ${active ? 'text-white shadow-sm' : 'bg-white text-gray-500'}`}
                  ...
                  {members.map((m) => {
                    ...
                    return (
                      <button
                        ...
                        className={`flex items-center gap-2 rounded-full border px-2.5 py-1 text-[10px] font-bold transition-all ${isActive ? 'text-white shadow-sm' : 'bg-white text-gray-600'}`}
        ...
        <div className="space-y-3 rounded-2xl border border-violet-100 bg-white p-3 shadow-sm">
          <div className="flex items-center justify-between gap-3">
            <span className="text-[10px] font-black uppercase text-gray-500">Service +10%</span>
            <input
              type="number"
              ...
              className="w-20 rounded-xl border border-violet-100 bg-white px-2 py-1 text-right text-xs font-bold text-gray-700 outline-none focus:ring-2 focus:ring-violet-400"
            />
          </div>
          <div className="flex items-center justify-between gap-3">
            <button
            ...
            >
              {(bill.id.startsWith('ocr-') ? results[parseInt(bill.id.split('-')[1]!, 10)]?.vatIncluded : manualBills.find((m) => m.id === bill.id)?.vatIncluded) ? 'VAT รวมแล้ว' : 'VAT แยก +7%'}
            </button>
            <input
              type="number"
              ...
              className="w-20 rounded-xl border border-violet-100 bg-white px-2 py-1 text-right text-xs font-bold text-gray-700 outline-none focus:ring-2 focus:ring-violet-400 disabled:bg-gray-50 disabled:text-gray-400"
            />
          </div>
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-black uppercase text-pink-500">Discount (-)</span>
            <input
              type="number"
              ...
              className="w-16 rounded-xl border border-pink-100 bg-white px-2 py-1 text-right text-xs font-black text-pink-600 outline-none focus:ring-2 focus:ring-pink-300"
            />
          </div>
        ...
        <div className="rounded-2xl border border-violet-100 bg-gradient-to-br from-violet-50/95 via-white to-fuchsia-50/80 p-3 shadow-[inset_0_1px_0_rgba(255,255,255,0.85)]">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              onClick={() => onSetPayer(bill.id, '')}
              className={`rounded-full border px-3 py-1.5 text-[10px] font-bold transition-all ${!assignedId ? 'border-violet-200 bg-violet-600 text-white shadow-sm' : 'border-white/80 bg-white/90 text-gray-500 hover:bg-gray-50'}`}
            >
              ยังไม่ระบุ
            </button>
            {members.map((m) => {
              ...
              return (
                <button
                  key={m.id}
                  onClick={() => onSetPayer(bill.id, m.id)}
                  className={`flex items-center gap-2 rounded-full border px-3 py-1.5 text-[10px] font-bold transition-all ${active ? 'border-transparent bg-violet-600 text-white shadow-sm' : 'border-white/80 bg-white/90 text-gray-600 hover:bg-gray-50'}`}
                >
                  <span className="flex h-4 w-4 items-center justify-center rounded-full text-[8px] font-black text-white" style={{ backgroundColor: m.color }}>
                    {m.name.slice(0, 1)}
                  </span>
